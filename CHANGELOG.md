...
- **报告下载 MIME 类型修复**
  - 修复 `ReportService::getDownloadData()` 对所有文件返回 `text/csv` 的问题
  - 现在根据文件扩展名正确返回 MIME 类型（.md → text/markdown, .csv → text/csv）

### ✨ 新功能

- **7月1日调研报告入库**
  - 新增 ID=28 的热销商品调研报告（化妆工具/手机配件/夏季清凉小物）
...
- **UI一致性修复**
  - Header：删除无功能搜索框，修复通知和用户模块右对齐
  - 执行日志：刷新按钮移到标题栏右侧，与错误日志/数据报告布局一致
  - 权限管理：状态标签统一为"正常/已禁用"，与设置页面保持一致
  - 设置页面：进入时自动刷新用户信息，显示最新的状态、注册时间、最后登录时间

### ✨ 新功能

- **调研报告数据入库**
  - 15份热销商品调研报告（6/8-6/30）同步到 reports 数据库表
  - 支持通过 API 查询和展示调研报告数据
...
- **真实系统监控**
  - 仪表盘"系统状态"现在显示真实的 CPU/内存/磁盘使用率
  - 后端 API 读取 `/proc/stat`、`/proc/meminfo` 获取系统信息
  - 显示 CPU 核心数、系统运行时间、内存/磁盘详细信息
  - 前端每 10 秒自动刷新系统状态
  - 使用率进度条根据阈值变色（<70% 绿色、70-90% 黄色、>90% 红色）

- **系统告警模块**（仅管理员）
  - 系统资源告警规则管理（CPU/内存/磁盘阈值）
  - 告警历史记录和处理
  - 支持 Webhook 和邮件通知
  - 冷却时间防止重复告警
  - 手动检测告警功能
